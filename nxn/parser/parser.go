...
func parseBody(parser *Parser, lexer lx.Lexer, i int) (int, error) {
	...
	k, e = matchToken(lexer, lx.TOKEN_RBRACE, k)
	return k, e
}
...
func parseStatements(parser *Parser, lexer lx.Lexer, i int) (int, error) {
	...
	// FIXME: This is good enough for now but I don't trust it.
	for level > 0 {
		next := Peek(lexer, k)
		switch next {
		case lx.TOKEN_LET:
		...
		case lx.TOKEN_INTVAL:
			k, e = matchToken(lexer, lx.TOKEN_INTVAL, k)
		case lx.TOKEN_IDVAL:
...
func parseExpressions(parser *Parser, lexer lx.Lexer, i int) (int, error) {
	...
	case lx.TOKEN_IDVAL:
		k, e = matchToken(lexer, lx.TOKEN_LPAREN, k+1)
		if e != nil {
			return k, e
		}
...
func parseLetAssignStmt(parser *Parser, lexer lx.Lexer, i int) (int, error) {
	...
	k, e = matchToken(lexer, lx.TOKEN_SEMICOLON, k)
	if e != nil {
		return k, e
	}
